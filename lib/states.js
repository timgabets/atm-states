...
class StatesService{
  ...
  check_version(){
    if(this.settings){
      ...
      if(settings_version){
        if(semver.lt(settings_version, this.pkg_version)){
          this.settings.set('states', {});
          if(this.log)
            this.log.error('atm_states module was updated from ' + settings_version + ' to ' + this.pkg_version + '. Please, upload ATM states from host');
        } else {
